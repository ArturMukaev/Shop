...
namespace Shop
{
    public partial class Souvenirs : Form
    {
        public Souvenirs()
        {
            InitializeComponent();
        ...
        {
            search.Text = "Искать в Gamer";
            search.ForeColor = Color.Gray;
            DB db = new DB();
            DataTable table = new DataTable();
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("SELECT * FROM `products` WHERE `products`.`id_cat` = 5", db.getConnection());
            ...
            if (lis.Count == 0)
            {
                MessageBox.Show("В данной категории нет товаров!");
                this.Close();
